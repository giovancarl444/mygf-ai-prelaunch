      ...
      <main id="top">
        <section className="map-hero" aria-labelledby="hero-title">
          ...
          <div className="hero-catalogue" aria-label="Companion world preview">
            ...
            <div className="catalogue-cards">{companionWorlds.slice(0, 3).map(world => <div className={`mini-world mini-${world.tone}`} key={world.name}><div className="zero-overlay-mini-media"><img src={world.image} alt={`Portrait assigned to fictional adult AI companion ${world.name}`} /></div><div className="mini-world-copy"><span>{world.category}</span><strong>{world.name.split(" ")[0]}</strong><p>{world.tag}</p></div></div>)}</div>
