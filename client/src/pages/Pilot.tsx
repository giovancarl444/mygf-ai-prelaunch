...
import { ArrowLeft, CircleAlert, Loader2, LockKeyhole, Sparkles } from "lucide-react";
...
export default function Pilot() {
  ...
  const [mapWorldSlug, setMapWorldSlug] = useState("sienna-vale");
  const [mapDisplayName, setMapDisplayName] = useState("Sienna Vale");
  ...
  const [draftFirstName, setDraftFirstName] = useState("Sienna");
  const [draftLastName, setDraftLastName] = useState("Vale");
  const [draftBiography, setDraftBiography] = useState("A clearly adult fictional AI companion world: bright, observant, lightly playful, and designed for imaginative private text threads.");
  const [draftGender, setDraftGender] = useState<"Female" | "Male">("Female");
  const [draftDateOfBirth, setDraftDateOfBirth] = useState("1998-05-19");
  ...
  const generateDraft = trpc.ohapiPilot.admin.generateDraft.useMutation({
    onSuccess: draft => {
      if (draft.characterGuid) setDraftGuid(draft.characterGuid);
    },
  });
